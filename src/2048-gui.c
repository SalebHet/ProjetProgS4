 ...
 */
typedef enum { MOVE_UP = UP, MOVE_LEFT = LEFT, MOVE_DOWN =
        DOWN, MOVE_RIGHT = RIGHT, RUN, GAME_OVER, QUIT } state;
...
Uint32 get_tile_color(SDL_PixelFormat * f, int tile) {
    ...
    if (tile == 0)
        ...
        b = 16 * fmax(0,
                      fmax(11 - fabs(4 - tile), 16 - fabs(18 - tile) - 1));
        r = 16 * fmax(11 - fabs(4 - tile), 16 - fabs(9 - tile) - 1);
        g = 16 * fmax(11 - fabs(4 - tile),
                      fmax(16 - fabs(9 - tile) - 1,
                           16 - fabs(14 - tile) - 1));
    }
...
void draw(vars_draw v, game g) {
    ...
    SDL_Surface *tile =
        SDL_CreateRGBSurface(SDL_HWSURFACE, 100, 100, 32, rmask, gmask,
                             bmask, amask);
        ...
        for (int y = 0; y < GRID_SIDE; y++) {
            ...
            if (t != 0) {
                ...
                if (size < 3)   //on adapte la taille de la police en fonction du nombre de caractères que contient la valeur de la tuile
                    ...
                    size_font = 3;
                SDL_Surface *s =
                    TTF_RenderText_Solid(v->fonts[size_font], str, c);
    ...
    if (g->st == GAME_OVER) {
        ...
        SDL_Surface *texte =
            TTF_RenderText_Blended(v->fonts[4], "GAME OVER !", noir);
